...
export class CndFldsPropertyTypeWorkspaceViewSettingsElement extends UmbLitElement implements UmbWorkspaceViewElement {
		...
		this.consumeContext(UMB_PROPERTY_TYPE_WORKSPACE_CONTEXT, (instance) => {
			...
			this.observe(instance?.data, (data) => {
				...
				if (this.#lastSavedPropertyId && this.#lastSavedPropertyId !== data?.unique && this._isDirty) {
					console.log('[ConditionalProperties] Switching properties, saving previous configuration');
			...
			this.observe(instance?.isNew, (isNew) => {
				...
				if (isNew === false && this._isDirty) {
					console.log('[ConditionalProperties] Workspace saved, persisting configuration');
					this.#saveConfiguration();
				}
			}, 'observeIsNew');
		});
	...
	async #loadConfiguration() {
		...
		try {
			const { data, error } = await ConditionalPropertiesService.getConfiguration({
				path: {
	...
	async #saveConfiguration() {
		...
		try {
			...
			const { error } = await ConditionalPropertiesService.saveConfiguration({
				path: {
					propertyTypeKey: this._currentPropertyKey
				},
				body: {
			...
			if (error) {
				...
				console.log('[ConditionalProperties] Configuration saved successfully');
				this._isDirty = false;
				this.#lastSavedPropertyId = this._currentPropertyKey;
			}
		} catch (error) {
